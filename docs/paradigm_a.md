...
exploitable. The repository also contains **paradigm B** — counterfactual
...
exactly. See [`paradigm_b.md`](paradigm_b.md) for what it does and
[`paradigm_b_roadmap.md`](paradigm_b_roadmap.md) for where it is going.
...
canonicalised observation
        ...
        +-- everything else -> feature-group encoders
        |                      (cards / board / players / pot+history / position)
        |
...
python -m paradigm_a.cli.infer --checkpoint checkpoints/latest.pt   # or no checkpoint for a random net
...
`train.py`, `paradigm_a/cli/evaluate.py` and `infer.py --show-spec` all print the exact
...
bias. `tests/paradigm_a/test_environment.py` pins both behaviours.
...
stores its `player_perspective`, and `tests/paradigm_a/test_self_play.py` asserts that
...
Those card slots do not reach the trunk as raw one-hots: the network pulls them
out of their groups and embeds them with the **shared card embedding**
(`common/nets.py`, the same module paradigm B uses). A card's index is
`rank * 4 + suit`, which is exactly the factorisation the embedding assumes, so
the outer product of the rank and suit one-hots reconstructs the card exactly —
the observation layout did not have to change to feed it. Slots are then summed
into *bags*: hole, flop, turn, river, and one per masked opponent. Order within
a bag cannot matter (two hole cards are a hand however you list them) while the
street a card arrived on is preserved. `ModelConfig.card_embedding_dim = 0`
restores the older flat encoding, and checkpoints written before the embedding
existed load that way automatically.
...
inspecting feature indices. `tests/paradigm_a/test_information_leakage.py` mutates the
...
paradigm_a/
  ...
  environment/     rules: betting, state, the env itself
  ...
  cli/             train, benchmark, evaluate, eval_population, infer

common/            shared with paradigm B: cards, hand evaluation, the card
                   embedding and the LayerNorm/GeLU trunk both networks use
tests/paradigm_a/  this paradigm's tests
train.py           a shim onto `paradigm_a/cli/train.py`
```

The other two functionalities live under `paradigm_b/` and are documented in
[`paradigm_b.md`](paradigm_b.md) and [`arms.md`](arms.md).

The `Agent` interface and `NetworkAgent` live in `paradigm_a/training/self_play.py` so the
...
Two variance reductions are built in, both in `paradigm_a/evaluation/evaluate.py`:
...
Two lessons, both now baked into `paradigm_a/evaluation/evaluate.py`: always report an
...
`paradigm_a/cli/benchmark.py` trains a matrix of configurations and scores each against the
...
```bash
python -m paradigm_a.cli.benchmark train    --config full --seed 0 --root runs/   # per cell
python -m paradigm_a.cli.benchmark evaluate --config full --seed 0 --root runs/
python -m paradigm_a.cli.benchmark report   --root runs/ --out runs/report        # tables + plots
python -m paradigm_a.cli.benchmark replot   --report runs/report                  # redraw figures
```
...
python -m paradigm_a.cli.benchmark train --config population --seed 0 --root runs/
```
...
Design and rationale live in `paradigm_a/training/population.py`.
...
The worker is `CoevolutionSelfPlayWorker` in `paradigm_a/training/self_play.py`.
...
Code: `paradigm_a/training/league.py` (population and management),
`paradigm_a/training/league_metrics.py` (the three metrics),
`paradigm_a/training/league_play.py` (the worker).

### Evaluating each network

`benchmark.py evaluate` only knows the named benchmark configs, so a co-evolution
run's per-network checkpoints are scored with `paradigm_a/cli/eval_population.py`.  It loads
...
```bash
python -m paradigm_a.cli.eval_population --checkpoint-dir checkpoints/pop5 --hands 2000
```
...
* **Environment** (`paradigm_a/environment/`): 3-handed no-limit hold'em with per-hand
  blinds and stacks, EV all-in runouts, a configurable bet abstraction, and
  strict information hiding (leakage tests in `tests/paradigm_a/test_information_leakage.py`).
* **Representation** (`paradigm_a/representation/`): structured features -> a fixed vector,
  canonicalised to the acting seat (SELF / OPPONENT_LEFT / OPPONENT_RIGHT).
* **Model** (`paradigm_a/model/`): per-group feature encoders -> a residual-MLP trunk ->
...
* **Training** (`paradigm_a/training/`): batched self-play fills a replay buffer; the
...
`stage1_medium`, `stage1_large` in `paradigm_a/cli/benchmark.py`.
...
```bash
python -m paradigm_a.cli.benchmark replot --report benchmark_results/alpha_sweep/report
python -m paradigm_a.cli.benchmark replot --report benchmark_results/ablation/report --out /tmp/ablation
```
...
```bash
python -m paradigm_a.cli.benchmark report --root benchmark_runs/ --out benchmark_runs/report
```
...
* The dashed line on the entropy panel is `log(mean legal actions)` -- the
  ...
  by default, ~5.4 with the finer grid), so `paradigm_a/cli/benchmark.py` sets it from the
