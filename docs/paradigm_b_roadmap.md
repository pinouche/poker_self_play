...
> Code: `paradigm_b/core/{game,cfr,belief,search}/` and
> `paradigm_b/leduc/{value_net,rebel}/`, driven by `solve.py`; 74 tests in
> `tests/paradigm_b/core/` and `tests/paradigm_b/leduc/`.  Full write-up with
> numbers and the findings that mattered: [`paradigm_b.md`](paradigm_b.md).
>
...
> `paradigm_b/holdem/arm1_fixed/` and `paradigm_b/holdem/arm2_iterative/` run
> the two regimes head to head at equal label and update budgets, and
> `paradigm_b/holdem/compare/relabel.py` measures the drift directly.  Start at
> [`arms.md`](arms.md).
...
- `paradigm_a/environment/` — the rules engine (betting, side pots, showdown), the hand
...
- `paradigm_a/model/residual_blocks.py`, `paradigm_a/model/heads.py` — the `FeatureGroupEncoder` +
...
- `paradigm_b/core/game/` — explicit **game-tree / information-set** representation, starting
  with a toy game (Leduc). Enumerable infosets, chance nodes, terminal payoffs.
- `paradigm_b/core/cfr/` — regret matching, CFR / CFR+ / MCCFR, average-strategy tracking, and an
  **exact best-response / exploitability** routine.
- `paradigm_b/core/belief/` — PBS representation and **Bayesian range propagation** through
  actions and chance.
- `paradigm_b/core/search/` — depth-limited re-solving over a subgame given a PBS.
- `paradigm_b/leduc/value_net/` — V(PBS) → counterfactual values per hand (reuse the trunk;
  inputs = pot, board, both range vectors; outputs = per-hand values / a value
  for each bucket).
- `paradigm_b/leduc/rebel/` — the ReBeL self-play+search training loop.
