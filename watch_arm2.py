...
def render(now: dict, previous: Optional[dict]) -> str:
    ...
    lines.append(
        f"[{time.strftime('%H:%M:%S')}] {alive}   elapsed {elapsed/3600:5.2f} h"
        + (f" of {hours:.0f} h ({100*elapsed/3600/hours:.1f}%)" if hours else "")
    )
    ...
    if labels and elapsed:
        overall = labels / elapsed
        remaining = max(hours * 3600 - elapsed, 0) if hours else 0
        lines.append(
            f"  labels {labels:>10,}   {overall:6.1f}/s overall"
