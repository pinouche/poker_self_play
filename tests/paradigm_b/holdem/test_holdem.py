...
def test_translation_is_exact_at_the_abstraction_sizes():
    from paradigm_b.holdem.engine.translation import bet_sizes_in_pots, translate_bet
...
def test_translation_leans_to_the_larger_size():
    ...
    """
    from paradigm_b.holdem.engine.translation import pseudo_harmonic_weight

    assert pseudo_harmonic_weight(0.5, 1.0, 0.75) < 0.5


def test_translation_is_randomised_but_reproducible():
    from paradigm_b.holdem.engine.translation import translate_action
...
def test_sampled_situations_differ():
    ...
    from paradigm_b.holdem.data.sampling import sample_situation
...
def test_sampled_ranges_are_valid_and_varied():
    from paradigm_b.holdem.data.sampling import RANGE_STYLES, sample_board, sample_range
...
def test_isomorphic_boards_share_a_canonical_form():
    from paradigm_b.holdem.engine.isomorphism import canonical_board
...
def test_there_are_1755_distinct_flops():
    ...
    from paradigm_b.holdem.engine.isomorphism import count_canonical

    assert count_canonical(3) == 1755


def test_strengths_and_ranges_survive_relabelling():
    from paradigm_b.holdem.engine.isomorphism import canonical_board, canonicalise_range, combo_permutation
...
def test_batched_river_solver_matches_solving_one_at_a_time():
    ...
    from paradigm_b.holdem.data.generation import river_state
    from paradigm_b.holdem.engine.public_tree import build_endgame_tree
    from paradigm_b.holdem.data.sampling import sample_range
...
def test_batched_showdown_matches_the_single_version():
    from paradigm_b.holdem.engine.showdown import showdown_values, showdown_values_batch
...
def test_generated_examples_are_well_formed():
    from paradigm_b.holdem.data.generation import GenerationConfig, generate
...
# --- layered data: bootstrapping the streets above the river ---------------
...
def test_bootstrapped_target_matches_an_exact_solve():
    ...
    from paradigm_b.holdem.engine.public_tree import build_endgame_tree
    from paradigm_b.holdem.data.sampling import sample_range
    from paradigm_b.holdem.net.leaf_values import RiverLeafValues
